...
library(reshape2)
nprec <- melt(precision, id.vars=1, variable.name = "System", value.name = "Value")
nrec <- melt(recall, id.vars=1, variable.name = "System", value.name = "Value")
npw <- melt(precision.watched, id.vars = 1, variable.name = "System", value.name = "Value")
```
...
ggplot(nprec, aes(x=k, y=Value, col=System)) + geom_line() + geom_point()
```
...
ggplot(nrec, aes(x=k, y=Value, col=System)) + geom_line() + geom_point()
```
...
ggplot(npw, aes(x=k, y=Value, col=System)) + geom_line() + geom_point()
```
