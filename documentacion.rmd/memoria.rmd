...
\Large \emph{Profesor:}\\
Enrique Herrera-Viedma\\[3cm] % Your name
...
En primer lugar realizamos una evaluación no supervisada de los sistemas de recomendación implementados. Para ello, utilizamos las siguientes medidas clásicas para sistemas de recomendación \cite{c3, c4}:
...
system.names <- c("Popularity",
                  ...
                  "Collaborative (SVD)",
                  "Collaborative (KNN)",
                  "Hybrid-cascade (overview, SVD)",
                  "Hybrid-cascade (overview, KNN)",
                  "Hybrid-cascade (c-g-k, SVD)",
                  "Hybrid-cascade (c-g-k, KNN)",
                  "Hybrid-weighted (overview, SVD)",
                  "Hybrid-weighted (overview, KNN)",
                  "Hybrid-weighted (c-g-k, SVD)",
                  "Hybrid-weighted (c-g-k, KNN)"
                  )
system.names = factor(system.names, levels = system.names, ordered = T)
no.superv <- data.frame(
  coverage                      = c(2.144e-4, 3.140e-2, 2.089e-2, 8.686e-3, 1.951e-3, 4.439e-2, 4.733e-2, 2.548e-2, 2.728e-2, 2.7560e-2, 2.3249e-2, 2.2134e-2, 1.9432e-2),
  personalization               = c(0.000000, 0.9521  , 0.9328  , 0.8477  , 0.4839  , 0.9567  , 0.9603  , 0.9532  , 0.9527  , 0.9543   , 0.9272   , 0.9512   , 0.9340   ),
  intralist.similarity.overview = c(1.017e-2, 5.089e-2, 2.204e-2, 8.608e-3, 1.098e-2, 4.196e-2, 4.123e-2, 2.417e-2, 2.491e-2, 2.6726e-2, 2.6744e-2, 2.2145e-2, 2.1935e-2),
  intralist.similarity.cgk      = c(8.786e-2, 7.197e-2, 0.2045  , 7.963e-2, 8.813e-2, 7.055e-2, 7.007e-2, 0.1979  , 0.2025  , 8.5521e-2, 8.7552e-2, 0.1480   , 0.1457   ),
  novelty                       = c(0.000000, 1.0974  , 1.4743  , 2.3698  , 2.4060  , 0.9289  , 0.8528  , 1.2155  , 1.2094  , 1.1313   , 1.5107   , 1.2641   , 1.5095   ),
  System = system.names,
...
ggplot(no.superv, aes(x = System, y=coverage, fill=System)) + geom_bar(stat = "identity") + scale_fill_manual(values=getPalette(13)) + theme(axis.text.x = element_blank()) + ylab("Coverage")
```

```{r, echo=F, warning=F, fig.cap="Personalización obtenida en los distintos sistemas de recomendación. \\label{fig:personalization}"}
library(ggplot2)
ggplot(no.superv, aes(x = System, y=personalization, fill=System)) + geom_bar(stat = "identity") + scale_fill_manual(values=getPalette(13)) + theme(axis.text.x = element_blank()) + ylab("Personalization")
```
...
library(ggplot2)
ggplot(no.superv, aes(x = System, y=intralist.similarity.overview, fill=System)) + geom_bar(stat = "identity") + scale_fill_manual(values=getPalette(13)) + theme(axis.text.x = element_blank()) + ylab("Intra-list similarity (overview)")
```
...
library(ggplot2)
ggplot(no.superv, aes(x = System, y=intralist.similarity.cgk, fill=System)) + geom_bar(stat = "identity") + scale_fill_manual(values=getPalette(13)) + theme(axis.text.x = element_blank()) + ylab("Intra-list similarity (c-g-k)")
```

```{r, echo=F, warning=F, fig.cap="Novedad obtenida en los distintos sistemas de recomendación. \\label{fig:novelty}"}
library(ggplot2)
ggplot(no.superv, aes(x = System, y=novelty, fill=System)) + geom_bar(stat = "identity") + scale_fill_manual(values=getPalette(13)) + theme(axis.text.x = element_blank()) + ylab("Novelty")
```
...
precision <- data.frame(
  ...
  hybrid.weighted.overview.svd = c(6.888e-3, 1.287e-2, 1.746e-2, 2.169e-2, 2.559e-2,
                                        2.855e-2, 3.133e-2, 3.469e-2, 3.746e-2, 3.961e-2,
                                        4.257e-2, 4.552e-2, 4.821e-2, 5.016e-2, 5.181e-2,
                                        5.336e-2, 5.551e-2, 5.808e-2, 6.013e-2, 6.296e-2),
  
  hybrid.weighted.overview.knn = c(5.037e-3, 1.056e-2, 1.412e-2, 1.727e-2, 2.043e-2,
                                        2.311e-2, 2.548e-2, 2.749e-2, 2.928e-2, 3.112e-2,
                                        3.261e-2, 3.456e-2, 3.634e-2, 3.809e-2, 4.035e-2,
                                        4.151e-2, 4.265e-2, 4.438e-2, 4.571e-2, 4.649e-2),
      
  hybrid.weighted.cgk.svd = c(8.317e-3, 1.488e-2, 2.114e-2, 2.6233e-2, 3.078e-2,
                                   3.579e-2, 4.007e-2, 4.310e-2, 4.731e-2, 5.096e-2,
                                   5.459e-2, 5.811e-2, 6.242e-2, 6.582e-2, 6.876e-2,
                                   7.195e-2, 7.526e-2, 7.897e-2, 8.300e-2, 8.640e-2),
      
  hybrid.weighted.cgk.knn = c(5.389e-3, 1.006e-2, 1.414e-2, 1.797e-2, 2.221e-2,
                                   2.631e-2, 2.948e-2, 3.399e-2, 3.715e-2, 4.049e-2,
                                   4.402e-2, 4.766e-2, 5.109e-2, 5.330e-2, 5.605e-2,
                                   5.958e-2, 6.222e-2, 6.541e-2, 6.882e-2, 7.070e-2)
...
precision.watched <- data.frame(
  ...
  hybrid.weighted.overview.svd = c(0.482, 0.655, 0.750, 0.796, 0.818,
                                        0.828, 0.834, 0.838, 0.839, 0.840,
                                        0.841, 0.841, 0.841, 0.842, 0.842,
                                        0.842, 0.842, 0.842, 0.842, 0.842),
  
   hybrid.weighted.overview.knn = c(0.427, 0.578, 0.645, 0.678, 0.698,
                                        0.709, 0.714, 0.716, 0.716, 0.717,
                                        0.717, 0.717, 0.717, 0.717, 0.717,
                                        0.717, 0.717, 0.717, 0.717, 0.717),
      
      hybrid.weighted.cgk.svd = c(0.441, 0.622, 0.714, 0.769, 0.797,
                                   0.815, 0.826, 0.832, 0.835, 0.837,
                                   0.838, 0.839, 0.839, 0.839, 0.839,
                                   0.839, 0.839, 0.839, 0.839, 0.839),
      
      hybrid.weighted.cgk.knn = c(0.429, 0.600, 0.681, 0.725, 0.750,
                                   0.765, 0.776, 0.783, 0.786, 0.787,
                                   0.788, 0.789, 0.789, 0.789, 0.789,
...
recall <- data.frame(
  ...
  hybrid.weighted.overview.svd = c(3.861e-3, 6.777e-3, 8.843e-3, 1.067e-2, 1.257e-2,
                                    1.391e-2, 1.512e-2, 1.672e-2, 1.783e-2, 1.853e-2, 
                                    1.964e-2, 2.096e-2, 2.222e-2, 2.318e-2, 2.355e-2,
                                    2.404e-2, 2.537e-2, 2.627e-2, 2.695e-2, 2.834e-2),
  
  hybrid.weighted.overview.knn = c(3.060e-3, 5.464e-3, 7.481e-3, 9.003e-3, 1.024e-2,
                                    1.136e-2, 1.211e-2, 1.285e-2, 1.358e-2, 1.458e-2,
                                    1.550e-2, 1.628e-2, 1.702e-2, 1.780e-2, 1.907e-2,
                                    1.947e-2, 2.007e-2, 2.087e-2, 2.128e-2, 2.148e-2),
  
  hybrid.weighted.cgk.svd = c(3.977e-3, 7.210e-3, 1.087e-2, 1.393e-2, 1.598e-2,
                               1.854e-2, 2.024e-2, 2.173e-2, 2.382e-2, 2.578e-2,
                               2.752e-2, 2.867e-2, 3.080e-2, 3.244e-2, 3.338e-2,
                               3.524e-2, 3.687e-2, 3.868e-2, 4.051e-2, 4.222e-2),
  
  hybrid.weighted.cgk.knn = c(2.581e-3, 5.248e-3, 7.306e-3, 9.631e-3, 1.203e-2,
                               1.430e-2, 1.550e-2, 1.821e-2, 1.946e-2, 2.049e-2,
                               2.185e-2, 2.327e-2, 2.490e-2, 2.573e-2, 2.710e-2,
                               2.817e-2, 2.940e-2, 3.131e-2, 3.265e-2, 3.336e-2)
...
\begin{figure}[h]
\centering
\includegraphics[width=7 cm]{./images/ex_inicial.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=7 cm]{./images/ex_set.png}
...
\begin{figure}[h]
\centering
\includegraphics[width=8 cm]{./images/ex_popularity.png}
...
\centering
\includegraphics[width=6 cm]{./images/ex_content_1}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
\centering
\includegraphics[width=6 cm]{./images/ex_content_2}
\end{minipage}\hfill
\begin{minipage}{0.3\textwidth}
\centering
\includegraphics[width=6.5 cm]{./images/ex_content_3}
\end{minipage}\hfill
...
\begin{figure}[h]
\centering
\includegraphics[width=8 cm]{./images/ex_collaborative.png}
...
\begin{thebibliography}{X}
	\bibitem{c1} R. Banik. \textit{The Movies Dataset. Metadata on over 45,000 movies. 26 million ratings from over 270,000 users.} \url{https://www.kaggle.com/rounakbanik/the-movies-dataset}
	\bibitem{c2} N. Hug. \textit{Surprise. A Python scikit building and analyzing recommender systems.} \url{http://surpriselib.com/}
	\bibitem{c3} C. Longo. \textit{Recmetrics. A Python library with diagnostic and evaluation metrics useful for evaluating recommender systems.} \url{https://github.com/statisticianinstilettos/recmetrics}
	\bibitem{c4} S. Vargas, \textit{Novelty and Diversity Enhancement and Evaluation in Recommender Systems (Trabajo de Fin de Máster)}. Universidad autónoma de Madrid, 2012.
\end{thebibliography}